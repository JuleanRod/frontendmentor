...
function getFetch() {
    ...
    const book = Object.keys(randomBook)[0];
    const chapters = Object.keys(randomBook[book]);
    
    let randomChapter = Math.floor(Math.random() * chapters.length);
    if (randomChapter == 0)  randomChapter = 1;
    const verses = randomBook[book][randomChapter];
    let randomVerse = Math.floor(Math.random() * verses);
    ...
    const url = `https://bible-api.com/${book} ${randomChapter}:${randomVerse}`
    console.log(url);
